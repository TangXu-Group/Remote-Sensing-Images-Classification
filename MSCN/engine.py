...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    device: torch.device, epoch: int, max_epoch, loss_scaler, max_norm: float = 0,
                    mixup_fn: Optional[Mixup] = None,
                    world_size: int = 1, distributed: bool = True, amp=True,
                    finetune=False,model_name=None
                    ):
    if finetune:
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        with torch.cuda.amp.autocast(enabled=amp):
            ...
            if model_name == "mgml":
                ...
                ratio = [1,0.5,0.2,0.5]
                los = 0.
                for out,crit,r in zip(outputs,criterion,ratio):
                    los += crit(out, targets) * r
                loss = los
            else:
...
@torch.no_grad()
def evaluate(data_loader, model, device, world_size, distributed=True, amp=False,model_name = None):
    ...
    for images, target in metric_logger.log_every(data_loader, 150, header):
